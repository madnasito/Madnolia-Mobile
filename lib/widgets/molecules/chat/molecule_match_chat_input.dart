...
class _MoleculeMatchChatInputState extends State<MoleculeMatchChatInput> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> usersLists = widget.match.joined
        .where((member) => member != userBloc.state.id)
        .toList();
    if (userBloc.state.id != widget.match.user) {
      usersLists.add(widget.match.user);
    }
    final bool isOwner = widget.match.user == userBloc.state.id;
    final bool isJoined = widget.match.joined.contains(userBloc.state.id);

    if (!isJoined && !isOwner) {
      return AtomJoinMatchButton(
        match: widget.match,
        onJoined: (newMatch) {
    ...
    return Wrap(
      children: [
        ...
        IconButton(
          style: IconButton.styleFrom(padding: const EdgeInsets.all(9)),
          icon: Icon(
            Icons.send,
            color: _text.isEmpty ? Colors.grey : Colors.blue,
          ),
          onPressed: () {
